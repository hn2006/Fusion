...
class ProcessReviewInvitationsJob(CronJobBase):
    ...
    Runs once a day and:
      ...
      5) If accepted (we’ll assume status still pending but last_sent set) → send daily review‐form link
    """
    RUN_EVERY_MINS = 0  # once daily
    schedule = Schedule(run_every_mins=RUN_EVERY_MINS)
    code = 'yourapp.process_review'
        ...
        for sub in submissions:
            ...
            for inv in invites:
                ...
                try:
                    ...
                    if inv.status == 'pending' and now >= inv.last_sent + timedelta(days=0):
                        send_invitation_email(inv)
                        inv.last_sent = now
                    ...
                    #    (we assume status remains 'pending' but professor clicked Accept)
